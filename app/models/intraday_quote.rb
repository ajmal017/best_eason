...
#  
#  scope :trade_on, -> (date) { where("trade_time >= ? and trade_time <= ?", date.at_beginning_of_day, date.at_end_of_day).order(trade_time: :desc) }
#  
...
#  end
#
#  def self.intraday_data(base_id, since_date)
#    IntradayQuote.by_base_id(base_id).select(:market, :trade_time, :volume, :last_trade_price_only, :local_date)
