...
namespace :resque do
  def rack_env
    fetch(:resque_rack_env) ||
      fetch(:rack_env) ||       # capistrano-rails doesn't automatically set this (yet),
  ...
  task :status do
    on roles(*workers_roles) do
      if test "[ -e #{fetch(:resque_pid_path)}/worker.1.pid ]"
        within current_path do
          files = capture(:ls, "-1 #{fetch(:resque_pid_path)}/worker*.pid")
      ...
      on roles(role) do
        ...
        workers.each_pair do |queue, number_of_workers|
          ...
          number_of_workers.times do
            pid = "#{fetch(:resque_pid_path)}/worker.#{worker_id}.pid"
            within current_path do
              execute :rake, %{RACK_ENV=#{rack_env} QUEUE="#{queue}" PIDFILE=#{pid} BACKGROUND=yes VERBOSE=1 INTERVAL=#{fetch(:interval)} #{"environment" if fetch(:resque_environment_task)} resque:work #{output_redirection}}
            end
            worker_id += 1
          end
        end
  ...
  task :stop do
    on roles(*workers_roles) do
      if test "[ -e #{fetch(:resque_pid_path)}/worker.1.pid ]"
        within current_path do
          pids = capture(:ls, "-1 #{fetch(:resque_pid_path)}/worker.*.pid").lines.map(&:chomp)
  ...
  namespace :scheduler do
    ...
    task :start do
      on roles :resque_scheduler do
        ...
        pid = "#{fetch(:resque_pid_path)}/scheduler.pid"
        within current_path do
          execute :rake, %{RACK_ENV=#{rack_env} PIDFILE=#{pid} BACKGROUND=yes VERBOSE=1 MUTE=1 resque:scheduler #{output_redirection}}
        end
      end
    end
